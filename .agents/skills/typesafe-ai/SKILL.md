...
| Decide how to handle uncertainty | [Confidence](https://docs.typesafe.ai/confidence.md), [Confidence-Gated Routing](https://docs.typesafe.ai/patterns/confidence-routing.md) |
...
### Primitives
- **`choice`**: Selects from mutually exclusive options. Returns `choice`, `probabilities` map, and `confidence`.
- **`noul`**: Evaluates yes/no. Returns `noul` (probability between 0 and 1).
- **`score`**: Ratings along ordered rubric levels. Returns `score`, `legend`, `probabilities`, and `confidence`.

### Confidence-Gated Routing Pattern
- Confidence acts as a second orthogonal axis: the answer tells you *what*; confidence tells you *whether to act*.
- Floor (< 0.60): Route to human or safe fallback.
- High-stakes automated action (> 0.85): Execute automatically.
- Moderate-stakes / ambiguous: Ask user/counsel to confirm or escalate to deeper reasoning.
